...
function Navigation({ button, setButton }) {
  ...
  return (
    <>
      <div
        className={`
          z-5 top-[56px] h-screen flex flex-col items-center gap-[2px] text-gray-500 font-bold w-screen max-w-[320px] bg-gradient-to-t from-[rgba(0,100,0,255)] via-green-600 to-[rgba(0,100,0,255)] absolute top-0 transition-all duration-500
          md:bg-none md:top-0 md:bg-[rgba(0,100,0,255)] md:relative md:left-0 md:pt-[56px] ${
            button ? "left-0 md:w-screen" : "-left-[100%] md:w-auto"
          }`}
      >
        <div className="text-white w-full p-3 flex items-center justify-center md:hidden">
        ...
        {links.map(
          (link) =>
            ...
            link.navigation.map((nav, index) => (
              ...
              >
                <div className="w-5 me-5 text-white">{nav.icon}</div>
                <h1 className={`bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent transition-all duration-500 ${button ? 'md:w-auto' : 'md:w-0'}`}>
                  {nav.title}
                </h1>
              </NavLink>
            ))
