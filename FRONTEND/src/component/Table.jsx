	...
	return (
		<div className="max-w-full overflow-x-auto">
			<table className='w-full border-collapse' role="table">
				<thead>
					<tr>
						<th className="max-[991px]:hidden text-white bg-[rgb(138,183,45)]">#</th>
						{tableHeadings.map((el, index) => {
							return (
								<th
									className={`text-white capitalize max-[991px]:hidden p-5 whitespace-nowrap bg-[rgb(138,183,45)] ${el === 'Action' ? 'text-center' : 'text-start'}`}
									key={index}>
									{el}
								</th>
							);
				...
				<tbody
					className='[&>*:nth-of-type(odd)]:bg-gray-300 [&>*:nth-of-type(even)]:bg-gray-200'>
					{data.map((el, index) => {
						return (
							<tr
								key={index}
								...
								{tableDataCell.map((dataCell, index) => {
									return (
										<td
											...
											className={`text-sm whitespace-nowrap hover:underline hover:decoration-red-500 ${dataCell === 'action' ? 'text-start lg:text-center' : 'text-start'}`}>
											{el[dataCell]}
										</td>
									);
								})}
